...
maps[a][b] = 2 # 방문한 칸 2로 표시

while True:
    move = False
    for _ in range(4): # 한 자리에서 최대 4방향 확인
        d -= 1 # 왼쪽으로 회전
        if d < 0:
            d = 3
        ...
        if 0 <= nx < n and 0 <= ny < m: # 맵 크기를 넘어가지 않고
            if maps[nx][ny] != 1 and maps[nx][ny] != 2: # 바다이거나 방문했던 칸이 아니면
                result += 1
                a = nx
                b = ny
                maps[nx][ny] = 2 # 방문 표시
    ...
    if move == True: # false - 4방향을 모두 검사했지만 갈 수 없는 경우
        continue

    ny = a - dx[d] # 뒤로 한 칸 이동
    nx = b - dy[d]
    if 0 <= nx < n and 0 <= ny < m:
        if maps[nx][ny] == 1: # 뒤로 이동 했는데 바다이면
            break # 멈춤
        else:
            a = nx
            b = ny
...
# 왼쪽으로 회전
def turn_left():
    global direction # direction 변수가 함수 바깥에서 선언된 전역변수이기 때문
    direction -= 1
    if direction == -1:
        direction = 3
