...
이때 왕실의 정원해서 행 위치를 표현할 때는 1부터 8로 표현하며, 열 위치를 표현할 때는 a부터 h로 표현한다.
...
for move in moves:
    if ord('a') <= ord(column) + move[0] <= ord('h'):
        if 1 <= row + move[1] <= 8:
            count += 1
...
'상하좌우' 문제에서는 dx, dy 리스트를 선언하여 이동할 방향 기록
이번 소스코드에서는 steps 변수가 dx와 dy 변수의 기능을 대신하여 수행
'''
