# 전보
# 난이도 상 | 풀이 시간 60분 | 시간 제한 1초 | 메모리 제한 128MB | 기출 유명 알고리즘 대회
...
각 도시의 번호와 통로가 설치되어 있는 정보가 주어졌을 때, 도시 C에서 보낸 메시지를 받게 되는 도시의 개수는 총 몇 개이며 도시들이 모두 메시지 받는 데까지 걸리는 시간은 얼마인지 계산하는 프로그램을 작성하시오.
'''
