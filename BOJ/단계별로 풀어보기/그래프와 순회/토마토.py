...
def bfs(graph):
    global count
    queue = deque()
    for i in range(n): # 익은 토마토가 있는 좌표를 모두 큐에 삽입
        for j in range(m):
            if graph[i][j] == 1:
                queue.append((i, j))
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0 <= nx < n and 0 <= ny < m:
                if not graph[nx][ny]: # 익지 않은 토마토가 있으면
                    graph[nx][ny] = graph[x][y] + 1 # 그래프에 현재 토마토 일 수 + 1 저장
                    queue.append((nx, ny))
...
for i in range(n):
    for j in range(m):
        if graph[i][j] == 0: # 익지 않은 토마토가 있으면 -1 출력
            print(-1)
            exit(0)
