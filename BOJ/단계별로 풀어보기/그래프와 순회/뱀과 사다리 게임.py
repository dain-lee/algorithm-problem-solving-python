...
- 1번 칸과 100번 칸은 뱀과 사다리의 시작 또는 끝이 아니다. 모든 칸은 최대 하나의 사다리 또는 뱀을 가지고 있으며, 동시에 두 가지를 모두 가지고 있는 경우는 없다.
...
def bfs(graph):
    queue = deque([1]) # 1번 칸에서 시작

    while queue:
        v = queue.popleft()
        if v == 100: # 100번 칸이면 탈출
            break
        for i in range(1, 7): # 주사위 1 ~ 6
            nv = v + i
            if nv <= 100:
                if graph[nv]: # 뱀이나 사다리가 있으면
                    nv = graph[nv] # 해당 칸으로 이동
                if not visited[nv]: # 처음 방문했으면
                    queue.append(nv) # 이동한 칸을 큐에 삽입
                    visited[nv] = visited[v] + 1 # visited 배열에 방문 순서 저장
...
for _ in range(n + m):
    a, b = map(int, input().split()) # 뱀과 사다리 정보 저장
    graph[a] = b
...
# 메모리 32440KB | 시간 92ms | 코드 길이 641B