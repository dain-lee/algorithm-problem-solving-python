...
min_cost = min(cost[:-1]) # 마지막 도시의 주유소 가격은 필요 없음
min_cost_idx = cost.index(min_cost)
total += sum(dist[min_cost_idx:]) * min_cost # 최소 가격의 주유소가 있는 도시에서 그 이후 가장 오른쪽 도시 까지의 거리만큼 주유

while min_cost_idx != 0:
    prev_idx = min_cost_idx # 이전 최소 가격 인덱스 저장
    min_cost = min(cost[:prev_idx]) # 가장 왼쪽 도시부터 이전 최소 가격 도시 사이에 있는 도시들 중에서 최소값 계산
    min_cost_idx = cost.index(min_cost)
    total += sum(dist[min_cost_idx:prev_idx]) * min_cost # 최소 가격 도시부터 이전에 계산한 최소 가격 도시까지의 거리만큼 주유

print(total)

# 58점 - min 함수 사용으로 인한 시간 초과
...
min_cost = cost[0] 
result += min_cost * dist[0] # 맨 처음 도시는 무조건 다음 도시까지의 거리만큼 주유

for i in range(1, n-1): # 첫번째 도시는 이미 주유, 마지막 도시에서는 주유 안함
    if cost[i] < min_cost: # 현재 도시의 가격이 이전의 최소 가격보다 작으면 교체
        min_cost = cost[i]
    result += min_cost * dist[i] # 다음 도시까지의 거리만큼 주유

print(result)

# 100점 | 메모리 45396KB | 시간 164ms | 코드 길이 332B