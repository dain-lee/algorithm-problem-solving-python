...
start = 1 # start = 0 이면 ZeroDivisionError
end = max(array)
...
while start <= end:
    ...
    for i in array:
        total += i // mid
    if total >= n: # n개보다 많이 만드는 것도 n개를 만드는 것에 포함되며, 최대 랜선 길이를 구해야 하므로 total == n일 때도 시작점을 증가시켜 이진 탐색
        result = mid
        start = mid + 1
    else:
        end = mid - 1
...
# 메모리 30840KB | 시간 492ms | 코드 길이 334B