...
for _ in range(n):
    student.append(tuple(map(int, input().split()))) # 몸무게, 키 튜플로 리스트에 저장

for i in student:
    rank = 1 # 1등부터 시작
    for j in student:
        if i[0] < j[0] and i[1] < j[1]: # 비교 대상의 몸무게와 키 값이 더 크면
            rank += 1
    result.append(rank)

print(*result) # 대괄호 없이 리스트 전체 출력

# 메모리 30840KB | 시간 72ms | 코드 길이 270B